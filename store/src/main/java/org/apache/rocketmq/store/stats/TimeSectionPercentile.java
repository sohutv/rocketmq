...
/**
 * 时间段百分位数，用于统计采样百分位数 采用非准确耗时统计，即将耗时进行分段，再统计耗时次数。
 * 共三段，其中0~10为一段，11~100为一段，100以上为一段，如下： [0~10]: 为精确统计 [11~15]: 统一为15ms [16~20]:
 * 统一为20ms 。。。 [96~100]: 统一为100ms [101~150]: 统一为150ms [151~200]: 统一为200ms 。。。
 * 之后类似
 * 
 * @author yongfeigao
 * @date 2018年9月10日
 */
public class TimeSectionPercentile {
    // 计数器
     ...
     * 构造方法
     * 
     * @param timeInMillis 预估的最大响应时间
     */
    public TimeSectionPercentile(int timeInMillis) {
    ...
    /**
     * 根据timeInMillis计算其所在的加标 例如12，对应的区域为[10,15], 下标为11 例如16，对应的区域为[16,20],
     * 下标为12
     * 
     * @param timeInMillis
     * @return
    ...
    /**
     * 将timeInMillis按照如下规则拆分成时间间隔: 1. 0~10与下标对应关系不变： 例如 [0~10]<->[0~10] 2.
     * 大于10小于100：按照15,20,25 ...，即每次按照5毫秒递增 例如 (10~15]<->11 3. 大于100：按照50毫秒递增 例如
     * (100~150]<->11
     * 
     * @param timeInMillis
     * @return 切分后的时间段数量
     ...
     */
    public int max() {
        return timeSectionStatsSampler.max();
    }
    ...
    public long getTotalCount() {
        return timeSectionStatsSampler.getTotalCount();
    }
     ...
     * @date 2018年9月10日
     */
    private class TimeSectionStatsSampler {
        // 快照数据
        ...
        public int max() {
            ...
            for (int i = sampledData.length - 1; i >= 0; --i) {
                if (sampledData[i] == 0) {
                    continue;
                }
                return time(i);
